...
class _ValidationCodeMailState extends State<ValidationCodeMail> {
  ...
  Future<void> demandeCode({String? email}) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Erreur envoi code'), backgroundColor: Colors.red),
      );
    } finally {
      setState(() {
        isResending = false;
            ...
            children: [
                ...
                child: TextField(
                  ...
                  decoration: const InputDecoration(
                    ...
                    focusedBorder: OutlineInputBorder(
                      borderRadius: BorderRadius.all(Radius.circular(10)),
                      borderSide: BorderSide(color: Color.fromARGB(255, 251, 7, 7), width: 2.0),
                    ),
                  ),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: isVerifying ? null : () => verificationCode(codeRecus: code),
                  ...
                  child: isVerifying
                      ...
                      : const Text('Envoyer', style: TextStyle(fontSize: 18, color: Colors.white)),
                ),
              ),
              if (verifyMessage != null)
                Padding(
                  ...
                  child: Text(
                    verifyMessage!,
                    style: TextStyle(
                      color: verifyMessage == "Vérification en cours..." ? Colors.black : Colors.red,
                      fontWeight: FontWeight.bold,
                    ),
                  ),
                ),
              const SizedBox(height: 40),
              const Text("Vous n'avez pas reçu le code ?", textAlign: TextAlign.center, style: TextStyle(fontSize: 23, fontWeight: FontWeight.bold)),
              const SizedBox(height: 10),
              const Text('Vérifiez vos spams ou cliquez sur "Renvoyer"', textAlign: TextAlign.center, style: TextStyle(fontSize: 18)),
              const SizedBox(height: 30),
              SizedBox(
                width: 350,
                child: ElevatedButton(
                  onPressed: isResending ? null : () => demandeCode(email: widget.mail),
                  style: ElevatedButton.styleFrom(
                  ...
                  child: isResending
                      ? const CircularProgressIndicator(color: Colors.white)
                      : const Text('Renvoyer un code', style: TextStyle(fontSize: 18, color: Colors.white)),
                ),
              ),
              if (resendMessage != null)
                Padding(
                  ...
                  child: Text(
                    resendMessage!,
                    style: TextStyle(
                      color: resendMessage!.contains("succès") ? Colors.green : Colors.red,
                      fontWeight: FontWeight.bold,
                    ),
                  ),
                ),
