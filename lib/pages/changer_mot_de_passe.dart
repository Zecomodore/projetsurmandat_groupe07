...
class _ChangerMotDePasseState extends State<ChangerMotDePasse> {
  ...
  String mpdConfirme = ''; // Variable pour stocker la confirmation du mot de passe
  ...
  void chagerMotDePasse({String? mdp, String? mpdConfirme}) async {
    ...
    final response = await dio.post("/changer",
      queryParameters: {
          'password': mdp,
    ...
    if (response.statusCode == 200) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Chamgement validé"), backgroundColor: const Color.fromARGB(255, 54, 244, 54)),
      );
      ...
      print("Erreur: $e");
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Chamgement échoué"), backgroundColor: Colors.red),
      );
    }
  }
            ...
            children: [
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: () {
                    chagerMotDePasse(mdp: mdp, mpdConfirme: mpdConfirme); // Appel de la méthode de changement de mot de passe
