  ...
  */
  ...
  void vahiculeDisponible() async {
    try {
      ...
      final response = await dio.post("/interventions/ajout/vehicule",
        queryParameters: {
  ...
  void vahiculeArrivee() async {
    try {
      ...
      final response = await dio.put("/interventions/arrive/vehicule",
        queryParameters: {
          'veh_use_id': PersonneVaraible().userId,
          'lsv_int_no': widget.idIntervention,
        },
  ...
  void vahiculeFinIntervention() async {
    try {
      ...
      final response = await dio.put("/interventions/supprimer/vehicule",
        queryParameters: {
          'veh_use_id': PersonneVaraible().userId,
          'lsv_int_no': widget.idIntervention,
        },
  ...
  void vahiculeEtat() async {
    try {
      ...
      final response = await dio.get("/interventions/etat/vehicule",
        queryParameters: {
          'veh_use_id': PersonneVaraible().userId,
          'lsv_int_no': widget.idIntervention,
        },
  ...
  void initState() {
    ...
    setState(() {
      chronometreLancer = (etat == ChronometreEtat.lancer || etat == ChronometreEtat.arreter);
              ...
              children: [
                Expanded(
                  child: ElevatedButton(
                    ...
                    child: const Text('Départ', style: TextStyle(fontSize: 18, color: Colors.white)),
                  ),
                ),
                ...
                Expanded(
                  child: ElevatedButton(
                    ...
                    child: const Text('Arrivée', style: TextStyle(fontSize: 18, color: Colors.white)),
                  ),
                ),
              ],
            ),
            SizedBox(height: screenHeight * 0.03),
            Text('Info : ${widget.type}', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
            const SizedBox(height: 10),
            Text('Heure : ${widget.heure}', style: const TextStyle(fontSize: 18)),
            /*
            const SizedBox(height: 10),
            ...
            */
            ...
            FractionallySizedBox(
              ...
              child: ElevatedButton(
                ...
                child: const Text('intervention terminée', style: TextStyle(fontSize: 18, color: Colors.white)),
              ),
            ),
          ],
        ),
