...
class _EspacePersonel extends State<EspacePersonel> {
  ...
  void chargerInfoUtilisateur() async {
    try {
      ...
      final response = await dio.get("/utilisateur/info/${PersonneVaraible().userId}");

      if (response.statusCode == 200) {
        List data = response.data; // Récupération des données de l'API
        
        if (data.isNotEmpty) { // Vérifier que la liste n'est pas vide
          setState(() {
            utilisrateur = data;
            info = '${utilisrateur[0]['uti_prenom']} ${utilisrateur[0]['uti_nom']}';
  ...
  void chargerInfoVehicule() async {
    try {
      ...
      final response = await dio.get("/vehicule/info/${PersonneVaraible().userId}");
  ...
  void changerVehiculeIndisponible() async {
    try {
      ...
      final response = await dio.put("/vehicule/indisponible",
        queryParameters: {
  ...
  void changerVehiculeDisponible() async {
    try {
      ...
      final response = await dio.put("/vehicule/disponible",
        queryParameters: {
          'veh_use_no': PersonneVaraible().userId,
        },
      );
  ...
  void changerUtilisateurIndisponible() async {
    try {
      ...
      final response = await dio.put("/utilisateur/indisponible",
        queryParameters: {
  ...
  void changerUtilisateurDisponible() async {
    try {
      ...
      final response = await dio.put("/utilisateur/disponible",
        queryParameters: {
          'uti_use_no': PersonneVaraible().userId,
        },
      );
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        iconTheme: const IconThemeData(
          color: Colors.white, // Définit la couleur des icônes de l'AppBar en blanc
        ),
      ),
      body: Padding(
              ...
              children: [
                Column(
                  ...
                  children: [
                    ...
                    Text(
                      role ?? 'Rôle non défini', // Affiche le rôle ou une valeur par défaut
              ...
              children: [
                ElevatedButton(
                  onPressed: () {
                    setState(() {
                      //isAvailable = true;
                      if (PersonneVaraible().nameType == 'ChefIntervention' || PersonneVaraible().nameType == 'SapeurPompier'){
